...
my answer:
SELECT a.name account, COUNT(o.id) order_count
FROM accounts a
JOIN orders o
ON a.id = o.account_id
GROUP BY  a.name
...
my answer:
SELECT a.name account, SUM(total_amt_usd) total_amt
FROM orders o
JOIN accounts a
...
my answer:
SELECT a.name account, COUNT(w.occurred_at) facebook_contact_count
FROM accounts a
JOIN web_events w
...
ORDER BY facebook_contact_count;
