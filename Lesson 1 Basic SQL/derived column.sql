...
my answer:
SELECT id, account_id, standard_amt_usd/(standard_qty+0.01) AS standard_unit_price
FROM orders
LIMIT 10;
...
This is for a division by zero. You will learn how to get a solution without an
error to this query when you learn about CASE statements in a later section.
For now, you might just add some very small value to your denominator as a work around.

my answer:
SELECT id, account_id,
        poster_amt_usd/(poster_amt_usd+gloss_amt_usd+standard_amt_usd+0.01) as poster_percentage
FROM orders;
