...
my answer:
SELECT CASE WHEN UPPER(LEFT(name, 1)) = LOWER(LEFT(name, 1)) THEN 'number'
            ELSE 'letter' END AS name_starts_with,
      COUNT(*)
FROM accounts
GROUP BY 1
...
my answer:
SELECT CASE WHEN LOWER(LEFT(name, 1)) IN ('a', 'e','i','o','u') THEN 'vowel'
            ELSE 'not vowel' END AS name_starts_with,
      COUNT(*)
FROM accounts
GROUP BY 1
